...
export const AchievementUnlocks = Object.freeze([
    ...
    "Defeat Isaac as???",
    "Die 100 times",
    "Pick up any 2 of the following items in a single run: Bob's Rotten Head, Dead Cat,\n                Cricket's Head, and Tammy's Head (Any 2 items that have the 'dead' item tag)",
    ...
    "Collect all non-DLC items in the game, and unlock all of the secrets and endings\n                (minus The Lost and his 6 unlockable items)",
    ...
    "Destroy 100 rocks",
    "Beat all Basement bosses (except Baby Plum) (not restricted to beating bosses in\n                The Basement)",
    "Beat all Caves bosses (except Bumbino) (not restricted to beating bosses in The\n                Caves)",
    "Beat all Depths bosses (except Reap Creep) (not restricted to beating bosses in\n                The Depths)",
    ...
    "Defeat Krampus",
    "Make a Super Meat Boy by picking up 4 copies of Cube of Meat in one run; see\n                Unlocking Super Meat Boy & Super Bandage Girl",
    "Destroy 100 Poops",
    "Pick up any 2 of the following items in a single run: Roid Rage, The Virus, Growth\n                Hormones, Experimental Treatment, and Speed Ball (Any 2 items that have the 'syringe' item tag)",
    ...
    "Collect every entry in the Bestiary (bugged v190 - Mobs that don't exist in the\n                bestiary are being counted. This allows Sin Collector to be unlocked without a complete bestiary)",
    "Participate in 31 Daily Challenges (they don't have to be consecutive; it will\n                still count if Isaac dies in the first room)",
    "Defeat The Lamb in under 20 minutes",
    "Defeat The Lamb without taking Hearts, Coins, or Bombs through the entire run.\n                Letting certain familiars (e.g. Bum Friend, Dark Bum, Bumbo or Lil Portal) or bosses (e.g. Bumbino) pick\n                up the consumables fails this condition as well. [ 1 ]",
    "Reset 7 runs in a row using the quick-reset key",
    "Complete a Chapter (floors I and II) after Basement, start-to-finish, with only\n                half a Heart total (can use The Lost)",
    "Obtain 50 items in a run (duplicate passive items/familiars count towards the\n                total, e.g., multiple Breakfast)",
    ...
    "Get a 5-win streak in the Daily Challenges (they don't have to be consecutive\n                days, only the runs you play are counted)",
    "Complete 3 Victory Laps by defeating The Lamb",
    "Defeat Delirium",
    "Collect every item in the game, unlock all secrets and endings, and complete the\n                Bestiary",
    "Defeat Mega Satan",
    ...
    "Increase in size 5 times in a single run via Magic Mushroom, \" One makes you\n                larger \" pills, XI - Strength, etc.",
    ...
    "Pick up any 2 of the following items/trinkets in a single run: 9 Volt, The\n                Battery, Car Battery, AAA Battery, and Watch Battery (Any 2 items that have the 'battery' item tag)",
    "Beat Chapter 3 without taking damage",
    "Pick up any 2 technology items in a single run (Any 2 items that have the 'tech'\n                item tag)",
    ...
    "Have Isaac die to his own explosion from Ipecac, Bob's Rotten Head, or a Horf!\n                pill",
    "Sleep in 10 beds",
    ...
    "Use Red Key to open the hidden closet in Home as Isaac",
    "Use Red Key to open the hidden closet in Home as Magdalene",
    "Use Red Key to open the hidden closet in Home as Cain",
    "Use Red Key to open the hidden closet in Home as Judas",
    "Use Red Key to open the hidden closet in Home as???",
    "Use Red Key to open the hidden closet in Home as Eve",
    "Use Red Key to open the hidden closet in Home as Samson",
    "Use Red Key to open the hidden closet in Home as Azazel",
    "Use Red Key to open the hidden closet in Home as Lazarus",
    "Use Red Key to open the hidden closet in Home as Eden",
    "Use Red Key to open the hidden closet in Home as The Lost",
    "Use Red Key to open the hidden closet in Home as Lilith",
    "Use Red Key to open the hidden closet in Home as Keeper",
    "Use Red Key to open the hidden closet in Home as Apollyon",
    "Use Red Key to open the hidden closet in Home as The Forgotten",
    "Use Red Key to open the hidden closet in Home as Bethany",
    "Use Red Key to open the hidden closet in Home as Jacob and Esau",
    ...
    "Earn all 12 Hard mode Completion Marks with all 17 standard characters",
    ...
    "Defeat Mother",
    "Earn all 12 Hard mode Completion Marks for all 34 characters (Standard & Tainted)",
...
export function getAchievementName(id: number): string {
    return AchievementNames[id] ?? `Achievement_${id}`;
}
...
export function getAchievementId(name: string): number {
    const idx = AchievementNames.findIndex(n => n.toLowerCase() === name.toLowerCase());
    return idx > 0 ? idx : -1;
}

export function getAchievementSprite(id: number, name?: string): string {
    const achName = name ?? getAchievementName(id);
    const id3 = String(id).padStart(3, '0');
    const safeName = achName
        .replace(/&nbsp;/g, ' ')
        .replace(/[<>:"/\\|*?]/g, '_')
        .replace(/\s+/g, '_')
        .trim();
